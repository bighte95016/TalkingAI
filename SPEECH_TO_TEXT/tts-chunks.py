...
def segment_text_by_sentence(text):
    ...
    sentence_boundaries = re.finditer(r'(?<=[.!?])\s+', text)    #透過找到分段標點符號後面的空格來分段
    ...
    for boundary_index in boundaries_indices:
        segments.append(text[start:boundary_index].strip())  #針對每個分段內容，把首尾空格去除，並加入segments (不含最後一個句子)
        start = boundary_index
    segments.append(text[start:].strip())                    #添加最末尾的句子
